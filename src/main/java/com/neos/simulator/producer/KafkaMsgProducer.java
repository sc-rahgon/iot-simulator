 ...
 * @author andrewserff
 */
public class KafkaMsgProducer extends EventProducer {

	 private static final Logger log = LogManager.getLogger(KafkaMsgProducer.class);
    ...
    public KafkaMsgProducer(Map<String, Object> producerConfig) {
    	super();
    	
    	String brokerHost = (String) producerConfig.get(BROKER_SERVER_PROP_NAME);
        Integer brokerPort = (Integer) producerConfig.get(BROKER_PORT_PROP_NAME);
        ...
        if (producerConfig.get(KERBEROS_CONF) != null) {
            Map<String, String> kerberosConf = (Map<String, String>) producerConfig.get(KERBEROS_CONF);
        ...
        this.topic = (String) producerConfig.get("topic");
        if (producerConfig.get("sync") != null) {
            this.sync = (Boolean) producerConfig.get("sync");
        } else {
            this.sync = false;
        }

        if (producerConfig.get("flatten") != null) {
            this.flatten = (Boolean) producerConfig.get("flatten");
        } else {
            this.flatten = false;
        }
